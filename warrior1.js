function Warrior1()
{
    ...
    this.ChooseBaseAction = function()
    {
        ...
        else if(this.myUnitCounts[UnitTypeEnum.TANK] < 2 && this.canAfford(UnitTypeEnum.TANK) && this.totalmyUnits > 25)
        {
            ...
            (this.enemyUnitCounts[UnitTypeEnum.SNIPER] < 1 || (this.totalmyUnits > 15 && this.enemyUnitCounts[UnitTypeEnum.SNIPER] < 3 ||
                this.totalenemyUnits > 20))
            && (this.enemyUnitCounts[UnitTypeEnum.TANK] < 1 || this.totalenemyUnits > 30)
            && current - this.warplan.startTime > 10000)
        {
            ...
            || this.enemyUnitCounts[UnitTypeEnum.SWARM] > 15) && 
            ((this.enemyUnitCounts[UnitTypeEnum.SNIPER] < 3 && this.enemyUnitCounts[UnitTypeEnum.TANK] < 1) 
                || (this.totalenemyUnits > 20 && this.totalenemyUnits - this.totalmyUnits < 8)))
        {
        ...
        /*else if(this.myUnitCounts[UnitTypeEnum.BRAWLER] < 15 && this.canAfford(UnitTypeEnum.BRAWLER) && 
        ...
        else if(this.enemyUnitCounts[UnitTypeEnum.GATLING] > 0 && this.myUnitCounts[UnitTypeEnum.SNIPER] < 5)
        {
        ...
        {
            //we're roughly evenly matched and I can afford a sniper...
            this.warplan.myBase.actionDescription = new ActionDescription(ActionEnum.SPAWN, UnitTypeEnum.SNIPER);      
        }
        else if(this.canAfford(UnitTypeEnum.GUNNER) && this.enemyUnitCounts[UnitTypeEnum.SWARM] > 8)
        {
        ...
        for(var i = 0; i < this.warplan.myUnits.length; i++)
        {
            ...
            for(var j = 0; j < this.warplan.enemyUnits.length; j++)
            {
                ...
                if(distance < this.warplan.enemyUnits[j].range + buffer &&
                    this.warplan.enemyUnits[j].unitType != UnitTypeEnum.SCOUT)
                {
            ...
            if(this.myUnitCounts[UnitTypeEnum.SWARM] < (this.enemyUnitCounts[UnitTypeEnum.SNIPER] + this.enemyUnitCounts[UnitTypeEnum.GUNNER]) &&
                this.totalmyUnits < this.warplan.myBase.maxUnitCount * 3 / 5)
            {
                //RETREAT!!!
                this.swarmAttacking = false;
            }
            ...
            if(
              ...
              && this.totalmyUnits < this.warplan.myBase.maxUnitCount - 2)
            {
                //go to my own base
            ...
            if((this.myUnitCounts[UnitTypeEnum.SWARM] > this.enemyUnitCounts[UnitTypeEnum.SNIPER] * 2 && 
                this.enemyUnitCounts[UnitTypeEnum.GATLING] < 1) ||
                this.totalmyUnits > this.warplan.myBase.maxUnitCount - 3)
            {
    ...
    this.ChooseGatlingActions = function()
    {
        //TODO: if enemies are overlapping, choose ones that haven't been chosen!  Or at least choose a random one.
        for(var i = 0; i < this.warplan.myUnits.length; i++)
        {
            ...
            for(var j = 0; j < this.warplan.enemyUnits.length; j++)
            {
                ...
                if(this.warplan.enemyUnits[j].speed >= this.warplan.myUnits[i].speed &&
                    distance > this.warplan.myUnits[i].range)
                {
                    //If there's an enemy out of range who's faster than me, ignore them
                    continue;
                }
            ...
            if(closestUnit == null)
            {
                //go to my part way between my own base and enemy base
                //TODO: run away from snipers...
                this.warplan.myUnits[i].actionDescription = null;
                x = (3 * this.warplan.myBase.x + this.warplan.enemyBase.x)/4 - this.warplan.myUnits[i].x
                y = (3 * this.warplan.myBase.y + this.warplan.enemyBase.y)/4 - this.warplan.myUnits[i].y
    ...
    this.ChooseUnitActions = function()
    {
        for(var i = 0; i < this.warplan.myUnits.length; i++)
        {
            ...
            for(var j = 0; j < this.warplan.enemyUnits.length; j++)
            {
               ...
               if(this.warplan.myUnits[i].unitType != UnitTypeEnum.GUNNER || true)
                {
